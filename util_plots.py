...
def GP_plot_data(
    ...
    GP_plot_data = pd.DataFrame(columns=["ID", "Date", "Hour", "TotalFlow","Weekday","Predicted"])

    ############################################ GP ############################################
    # resid_df_train = pd.DataFrame(columns=["mean_res", "std_res"])
    # temp1 = []
    # temp2 = []

    temp = groups.get_group(slice_idx).copy
    t, y_pred_new = model_prep_gp(
            groups_obs=groups_new,
            slice_idx=slice_idx,
            kernel=kernel,
        )
    temp["Predicted"]=y_pred_new[:,0]
    # temp["Predicted"]=list(y_pred_new)
    
    GP_plot_data = pd.concat([GP_plot_data, temp])
    # y_true = groups.get_group(slice_idx).TotalFlow  
        # temp1.append(np.mean(y_pred_new)-np.mean(y_true))
        # temp2.append(np.std(y_pred_new)-np.std(y_true))
    # result_kernel = [np.sqrt(np.mean(np.square(temp1))),np.sqrt(np.mean(np.square(temp2)))]
    return GP_plot_data
...
def training_kernel(
    ...
    # result_kernel = [np.sqrt(np.mean(np.square(temp1))),np.sqrt(np.mean(np.square(temp2)))]
    return {
            "RMSE_mean": np.sqrt(np.mean(np.square(temp1))),
            "RMSE_std": np.sqrt(np.mean(np.square(temp2))),
        }

def training_kernels(training_lst, data, data_obs_new, kernels):
    # res = pd.DataFrame(columns=["RMSE_mean", "RMSE_std","kernel"])
    res = []
    for i in range(len(kernels)):
        temp = training_kernel(data=data, data_obs_new=data_obs_new, kernel=kernels[i],training_lst=training_lst)
        print(temp)
        # temp.append(i)
        res.append([temp["RMSE_mean"],temp["RMSE_std"],i])
    return res
...
#     ),
#     ConstantKernel(2, (2, 10.0))
#     * (DotProduct(sigma_0=0.01, sigma_0_bounds=(0.01, 10.0))),
...
# test the function that computes rmse for a given fraction and a given training list
# sampled_IDs = random.sample(range(400), 50)
# fraction_ind = [0.01]
...
#     training, validation = train_val_test(seed=123,train_size=math.floor(len(slice_p) * 0.5),validation_size=math.floor(len(slice_p) * 0.5),
#     slice_list=slice_p)
#     training_lst.append(training)
#     validation_lst.append(validation)
# slice_idx = (12, "Monday", 11)
# res = GP_plot_data(data, data_obs, kernels[1], slice_idx)
# print(type(res))
# for kernel in kernels:
    
#     result  = training_kernel(
#         data,
#         data_obs,
#         kernel,
#         training_lst[0],
...
# training_kernels(data, data_obs, kernels, training_lst[0])